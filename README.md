...
配置好指定交换机的snmp v2c 只读团体名称后，就可以定期轮询相应交换机上的arp数据，将获得的ip、mac地址及时间写进数据库；下一次没有轮询到的已知ip会被设置为离线状态。
...
## 4.2 安装python并配置虚拟环境
注意不要追新，使用python3.10可以完美规避安装环境报错问题
```
# 创建虚拟环境
python -m venv venv
# 进入虚拟环境
.\venv\Scripts\activate
# 作者建议使用miniconda安装python，使用介绍见作者博客https://kiraster.github.io/posts/f2cfb11.html
# 安装miniconda后运行Anaconda Prompt (miniconda3)
#创建一个名称为pyqt-base的虚拟环境（指定Python版本3.10）
conda create -n pyqt-base python=3.10
#激活虚拟环境
...
class SNMPConfig:
    # 当超过30分钟没有updated_at值更新，则available值置为False，注意代码此处要将REFRESH_TIME从1改为30，单位：分钟
    REFRESH_TIME = 30
    SNMP_DATA = [
